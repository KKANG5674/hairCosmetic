...
//location.href='./login/loginSessionCheck.jsp;';
	<% //세션이 null이면 
...
function loginSubmit( ) {
	...
	var form = document.morning_main_login;
if (windowWidth>=768)
{
	form.login_id2.value = "";
	form.login_pass2.value = "";
	if( form.login_id1.value == "" ) {
		alert( "이메일을 입력하세요" );
		form.login_id1.focus();

	return false;
	}

	if( form.login_pass1.value == "" ) {
		alert( "비밀번호를 입력하세요" );
		form.login_pass1.focus();

	return false;
	}
}else{
	
	form.login_id1.value = "";
	form.login_pass1.value = "";
	if( form.login_id2.value == "" ) {
		alert( "이메일을 입력하세요" );
		form.login_id2.focus();

	return false;
	}

	if( form.login_pass2.value == "" ) {
		alert( "비밀번호를 입력하세요" );
		form.login_pass2.focus();
...
function onEnterSubmit(){
	...
	if(keyCode==13) loginSubmit(document.morning_main_login);
}
//-->
</SCRIPT>
		...
		<div style="width: 100%; text-align: center">
			<ul>
				<li>
					...
					<div>
						<ul>
							<li class="sub_sub_text">로그인</li>	
						</ul>	
					</div>
					<div>
						<ul>
							<li class="sub_title_text">안녕하세요</li>								
						</ul>	
					</div>
				</li>	
			</ul>	
...
<div style="background-color: #f4f4f7;">
	...
	<div style="width:100%; max-width:740px; margin:0 auto; text-align:center; padding-top:60px;">
		<form method="post" action="./login/loginPro.jsp" name="morning_main_login" >
		...
		<div style="width:100%; width:295px; margin:0 auto;">

			<div style="font-size:13px; margin-top:20px; color: #4a4a4a; opacity: 0.5;">
				회원가입 시 <a href="m_page289b.html?ps_pname=page_member" style="text-decoration:underline;" target="_blank">개인정보 취급방침</a>, <a href="m_page74d7.html?ps_pname=page_private" style="text-decoration:underline;" target="_blank">전자상거래 표준약관</a>에<br>동의하는 것으로 간주합니다.
			</div>

		<div class="login_middle_line">
		   <ul>
			  <li style="float:left; border: solid 1px #979797; opacity:0.2; width:100%; max-width: 43%; margin-top:8px;"></li>
			  <li style="float:left; color: #191919; font-size:18px; font-weight:900; width:12%;">or</li>
			  <li style="float:left; border: solid 1px #979797; opacity:0.2; width:100%; max-width: 43%; margin-top:8px;"></li>
			  <div style="clear:both;"></div>
		   </ul>
		</div>

		<div class="login_title">이메일로 로그인</div>
		...
		<div id="pc" style="max-width:295px; margin: 0 auto; padding-top:30px;">
		   <ul>
			  <li>
				 <div>
					<ul>
					   <li class="login_input"><input type="text" name="login_id1" id="login_id1" tabindex="1" placeholder="이메일 입력" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 1opacity: 0.2;"></li>
					</ul>
				 </div>

				 <div>
					<ul>
					   <li class="login_input"><input type="password" placeholder="비밀번호 입력" onkeypress="if(event.keyCode==13) {check_main_login();}" name="login_pass1" tabindex="2" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 1opacity: 0.2;"></li>
					</ul>
				 </div>
			  </li>
		   </ul>
		...
		<div id="mobile" style="max-width:295px; margin: 0 auto; padding-top:30px;">
		   <ul>
			  <li>
				 <div>
					<ul>
					   <li class="login_input"><input type="text" name="login_id2" id="login_id2" tabindex="1" placeholder=" 이메일 입력" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;"></li>
					</ul>
				 </div>

				 <div>
					<ul>
					   <li class="login_input"><input type="password" placeholder=" 비밀번호 입력" onkeypress="if(event.keyCode==13) {check_main_login();}" name="login_pass2" tabindex="2" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;"></li>
					</ul>
				 </div>
			  </li>
		   </ul>
		...
		<div style="max-width:295px; margin: 0 auto; margin-top:20px;">
			<ul>
				<li style="float:left; width:50%; text-align:left;">
					<a href="passSearch.jsp" class="login_pass_fail">비밀번호 찾기</a></li>
				<li class="ex_show" style="float:left; width:50%; text-align:right;">
					<span class="login_pass_fail">회원가입</span>
				</li>
			</ul>
		</div>
		...
		<div style="margin-top:60px; padding-bottom:60px;">
		   <ul>
			  <li class="login_btn_ok"><a href="javascript:loginSubmit()" style="color:#fff; font-size:14px; padding:14px 118px;">로그인</a></li>
		   <ul>
		</div>
		...
		<!--div class="ex_box" -->
		<div class="ex_box" style="display:none">
		<div class="login_title">회원가입</div>

		

        <form method="post"  name="morning" id="joinData">
		<input type="hidden" name="ps_ssl" value="">
		...
		<div style="max-width:700px; margin: 0 auto; padding-top:30px; text-align: left;">
		   ...
		   <ul>
			  <li>
				 <div>
					<ul>
					   <li class="login_input">
						<input type="text" name="id" id="id" tabindex="1" placeholder="이메일 입력" style="border:0px; width:175px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >
						...
						<div id="emailCheckMsg" class="error">아이디 중복 검사를 반드시 실행해 주세요.</div>
					   </li>
					</ul>
				 </div>
				 ...
				 <div>
					<ul>
					   <li class="login_input">
						<input type="text" name="name" id="name" tabindex="1" placeholder="이름 입력" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >
				 ...
				 <div>
					<ul>
					   <li class="login_input">
					   <input type="text" name="address" id="address" tabindex="1" placeholder="주소 입력" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >
				 ...
				 <div>
					<ul>
					   <li class="login_input">	
						...
						<input type="radio" name="sex" id="man" value="남" placeholder="남" style=" border:0px; width:20px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >남
						<input type="radio" name="sex" id="woman" value="여" placeholder="여" style=" border:0px; width:20px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >여	   
				 ...
				 <div>
				 	<ul>
					   <li class="login_input">
						 	...
						 	<input type="text" name="birthday" tabindex="1" placeholder="생년월일 입력   ex) 19910101(8자리)" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" id="birthday" >		 	
				 ...
				 <div>
					<ul>
					   <li class="login_input">
						<input type="text" name="phone" id="phone" tabindex="1" placeholder="휴대폰번호 입력  ex) 01011113333" style="border:0px; width:295px; background:#f4f4f7; font-size:16px; color: #191919; 2opacity: 0.2;" >
		...
		<div style="margin-top:60px; padding-bottom:60px;">
		   <ul>
			  <li class="login_btn_ok"><a href="javascript:function_user_insert();" style="color:#fff; font-size:14px; padding:14px 90px;">회원가입</a></li>
		   <ul>
		</div>
		...
		<script>
		window.onload = new Function( "morning_main_login.login_id1.focus();");
		...
		$("#idCheck").click(function() {
			...
			window.open("<%=request.getContextPath()%>/codeforbeauty.com/shop/login_id_check.jsp?id="+$("#id").val(),"아이디중복검사","width=450,height=300,left=450,top=250");
		});
