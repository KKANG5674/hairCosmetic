...
public class QuestionDAO extends JdbcDAO {
	...
	public List<QuestionDTO> getpageList(int startRow, int endRow, String search, String keyword) {
		...
		try {
			...
			if (keyword.equals("")) {
				String sql = "select * from (select rownum rn, temp.* from (select * from questiontable order by num desc) temp) where rn between ? and ? and title!='삭제글' order by num desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, startRow);
				pstmt.setInt(2, endRow);
			} else {
				String sql = "select * from (select rownum rn, temp.* from (select * from questiontable where " + search
						+ " like '%'||?||'%' order by num desc) temp) where rn between ? and ? and title!='삭제글' order by num desc ";
				pstmt = con.prepareStatement(sql);
...
//	
	...
	public List<QuestionDTO> selectId(String Id) {
		...
		try {
			...
			String query = "select * from questiontable where id=? and title!='삭제글' order by num desc ";
			pstmt = con.prepareStatement(query);
	...
	public int getQuestionTotal(String search, String keyword) {
			...
			try {
				...
				if(keyword.equals("")) {//검색 기능 미사용
					String sql="select count(*) from questiontable where title!='삭제글' order by num desc";
					pstmt=con.prepareStatement(sql);
				} else {//검색 기능 사용
					//컬럼명을 저장한 Java 변수는 InParameter로 사용 불가능
					String sql="select count(*) from questiontable where "+search+" like '%'||?||'%' and title!='삭제글' order by num desc";
					pstmt=con.prepareStatement(sql);
					pstmt.setString(1, keyword);
				}
	...
	public QuestionDTO getBoard(int num) {
		...
		try {
			...
			String sql="select * from questiontable where num=? and title!='삭제글' order by num desc";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
